...
static HWRTextureFormat PNG_Load(const char *filename, int *w, int *h, GLPatch_t *grpatch)
{
	png_structp png_ptr;
...
static HWRTextureFormat PCX_Load(const char *filename, int *w, int *h,
	GLPatch_t *grpatch)
{
	PcxHeader header;
#define PALSIZE 768
...
boolean HWR_DrawModel(gr_vissprite_t *spr)
{
	...
	// Look at HWR_ProjectSprite for more
	{
		...
		if ((gpatch && gpatch->mipmap->format) // don't load the blend texture if the base texture isn't available
			&& (!md2->blendgrpatch || !((GLPatch_t *)md2->blendgrpatch)->mipmap->format || !((GLPatch_t *)md2->blendgrpatch)->mipmap->downloaded))
